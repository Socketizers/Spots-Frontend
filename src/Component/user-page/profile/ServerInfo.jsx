...
function ServerInfo(props) {
  ...
  return (
    <Modal show={props.open} onHide={props.handleClose} size="lg">
      ...
      <img src={logo} width="60" style={{marginRight:'1em'}}/>
        <Modal.Title>{props.server.name} <span>{props.server.id}</span></Modal.Title>
          ...
          <Row style={{ margin: "2vh 5vh", height: "30vh" }}>
            <div className="rooms">
              ...
              {props.rooms.map((room, i) => {
                  ...
                  <button onClick={addRoomModalOpen}><i class="fas fa-plus-circle add-room"></i></button>
                  </>
                   :
                  <button onClick={addRoomModalOpen}><i class="fas fa-plus-circle add-room"></i></button>
                }
                  <CreateRooms id={props.server.id} open={openRoomModal} handleClose={addRoomModalClose} closeServerModal={props.handleClose}/> 
